...
This folder documents the **Planner** task system. The **structured source of truth** lives in **`data/tasks/`** (index.yaml, details/, task-schema.json). Use it for tracking, search, and project context across apps (e.g. primary-app, tasks-app).

Read **`TASK_INDEX_GUIDE.md`** first — it explains how to approach the index (purpose, refine, link related files, review, add new, implement). For **creating new tickets** that add value and improve the app (and optionally implementation details), see **`TASK_ORIENTATION.md`**.

## Files

| File / folder | Purpose |
|---------------|--------|
| **`TASK_INDEX_GUIDE.md`** | **Entry point:** how to approach index.yaml (purpose, refine tasks, link related files, review, add new, implement). |
...
| **`data/tasks/details/`** | Per-task details: `details/<id>-<slug>.md`. Add a `details` field in index.yaml pointing to the file. Read this first when working on a task. |
| **`details/_template.md`** | Template to copy when creating a new details file. |
| **`projects/`** | One .md per **project** (e.g. primary-app, tasks-app) describing scope. Filter by project in the tasks app. |
...
| **`CLARITY_SCORE.md`** | How **clarity_score** (0–100) works: evaluates implementation clarity from the details file; low = needs discovery, high = ready to implement. |
| **`README.md`** | This file. |
...
- **project** — Project scope: `primary-app` (main product) or `tasks-app` (improvements to the task list app). Lets you keep tasks-app backlog separate from main backlog.
...
- **start_date** — Optional. Start date `YYYY-MM-DD`. Backfilled for done tasks; UI or manual edits can set for new work.
- **due_date** — Optional. Due date `YYYY-MM-DD`. Backfilled for done tasks; UI or manual edits can set for new work.
- **clarity_score** — Optional. Integer 0–100. Indicates how clear the implementation path is based on the task’s details file. Low = needs discovery; high = ready to implement. See **`CLARITY_SCORE.md`**.
...
4. **Add context** — Use the **Notes** section in the task’s details file.
5. **Finish** — Set `status: done` in index.yaml.

## For search

- **List open tasks:** `status` in `[todo, in_progress]`.
- **By project:** Filter `project` (e.g. `primary-app`, `tasks-app`).
...
- **Clarity score:** After reading a task’s details file, **evaluate whether the implementation steps are clear** and set or update `clarity_score` (0–100) in index.yaml. See **`CLARITY_SCORE.md`** for how to interpret.
